...
from modeling.lance.modeling_utils import MLPconnector, PositionEmbedding, PositionEmbedding3D, TimestepEmbedder
from modeling.lance.qwen2_navit import Qwen2DecoderLayer, Qwen2MoEDecoderLayer, Qwen2MoTDecoderLayer
from modeling.vit.qwen2_5_vl_vit import Qwen2_5_VLVisionBlock, Qwen2_5_VisionTransformerPretrainedModel
...
def fsdp_wrapper(original_model, fsdp_config, ignored_modules=None):
    ...
    return FSDP(
        ...
        device_mesh=_device_mesh(fsdp_config),
    )


def _drop_shape_dependent_buffers(state_dict):
...
class FSDPCheckpoint:
    ...
    def fsdp_save_ckpt(
        ...
        with FSDP.state_dict_type(model, StateDictType.LOCAL_STATE_DICT):
            ...
            optimizer_save_path = os.path.join(save_path, f"optimizer.{shard_index:05d}-of-{total_shards:05d}.pt")
            if fsdp_config.sharding_strategy != "HYBRID_SHARD" or dist.get_rank() < fsdp_config.num_shard:
    ...
    def try_load_ckpt(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        ...
        model_file = "ema.safetensors" if resume_from_ema else "model.safetensors"
        model_state_dict_path = os.path.join(resume_from, model_file)
        model_state_dict = _drop_shape_dependent_buffers(load_file(model_state_dict_path, device="cpu"))
        ...
        if ema_model is not None:
            ...
            ema_state_dict = _drop_shape_dependent_buffers(load_file(ema_state_dict_path, device="cpu"))
    ...
    def try_load_train_state(resume_from, optimizer, scheduler, fsdp_config):
        ...
        optimizer_state_dict_path = os.path.join(resume_from, f"optimizer.{shard_index:05d}-of-{total_shards:05d}.pt")
        optimizer.load_state_dict(torch.load(optimizer_state_dict_path, map_location="cpu", weights_only=True))

        scheduler_state_dict_path = os.path.join(resume_from, "scheduler.pt")
        if os.path.exists(scheduler_state_dict_path):
            scheduler.load_state_dict(torch.load(scheduler_state_dict_path, map_location="cpu", weights_only=True))
        ...
        if os.path.exists(data_status_path):
            ...
            if isinstance(data_status, list) and dist.get_rank() < len(data_status):
                data_status = data_status[dist.get_rank()]
...
def fsdp_ema_setup(ema_model, fsdp_config, ignored_modules=None):
    ...
    return fsdp_wrapper(ema_model, fsdp_config, ignored_modules=ignored_modules or [])
